...
export default {
  ...
  methods: {
    drawLineFirst(res) {
      ...
      myChart.clear();
      let b_rate = 100*(res.Q[0]/this.$store.state.common.real_num);
      let g_rate = 100*(res.Q[1]/this.$store.state.common.real_num);
      console.log(b_rate);
      // 绘制图表
      myChart.setOption({
        title: {
        ...
        series: [
          { type: "bar", barWidth: 60,label:{show:true,position:'top',formatter:(para)=>{return para.value[1]+'%'}} },
          { type: "bar", barWidth: 60,label:{show:true,position:'top',formatter:(para)=>{return para.value[1]+'%'}} }
        ]
      });
    },
    ...
    drawLineFourth(res) {
      ...
      res.red_list_name.map(item => {
        series.push({ type: "bar", barWidth: 60 });
      });
      // 绘制图表
      myChart.setOption({
        title: {
        ...
        dataset: {
          source: [
            ...
            ["达标"].concat(100*(res.red_list_up_count/this.$store.state.common.real_num)),
